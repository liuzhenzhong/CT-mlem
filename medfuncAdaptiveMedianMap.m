function F = medfuncAdaptiveMedianMap( W_ind,W_dat,N,F0,P,irt_num ,beta)
...
%     F(ind) = F(ind).*(back_proj(ind)./(back_proj0(ind)+beta*((F(ind)-img(ind))./img(ind))+beta*U(ind)));% 基于中值先验的重建
    F(ind) = F(ind).*(back_proj(ind)./(back_proj0(ind)+5*temp(ind)+beta*U(ind)));
    k=k+1;   
    % 更新beta值  自适应更新beta值
    ...
    writebin(['.\result\mix\mix_' num2str(k) '.bin'],result_save);  
    %% 完成一次迭代进行一次滤波处理 
